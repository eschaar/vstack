...
class TestNormalizeTargetedNames:
    ...
    def test_returns_empty_set_for_none(self) -> None:
        ...
        assert BaseCommand._normalize_targeted_names(None) == set()

    def test_returns_empty_set_for_empty_list(self) -> None:
        """Empty list produces an empty set."""
        assert BaseCommand._normalize_targeted_names([]) == set()

    def test_strips_whitespace(self) -> None:
        """Names with surrounding whitespace are stripped."""
        assert BaseCommand._normalize_targeted_names(["  vision  ", " debug "]) == {
            "vision",
            "debug",
        }
    ...
    def test_deduplicates_names(self) -> None:
        ...
        assert BaseCommand._normalize_targeted_names(["vision", "vision", "debug"]) == {
            "vision",
            "debug",
        }
    ...
    def test_filters_blank_strings(self) -> None:
        ...
        assert BaseCommand._normalize_targeted_names(["  ", "", "debug"]) == {"debug"}

    def test_returns_set_of_names(self) -> None:
        """Result is always a set."""
        result = BaseCommand._normalize_targeted_names(["a", "b"])
